                                              ...
                                              float duration)
{
    ...
    while(iterationsUsed < iterations) {
        ...
        //if(updatePenetration) updateInterpenetrations(contactArray, numContacts, maxIndex);
        iterationsUsed++;
    }

}

/*void ParticleContactResolver::updateInterpenetrations(ParticleContact *contactArray, unsigned numContacts, unsigned maxIndex) {
    ...
    for (int i = 0; i < numContacts; i++) {
        ...
        if (contactArray[i].particle[1]) {
            ...
            else if (contactArray[i].particle[i] == contactArray[maxIndex].particle[1]) {
                contactArray[i].penetration += move[1] * contactArray[i].contactNormal;
            }
        }
    }
